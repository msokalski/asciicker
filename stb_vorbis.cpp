...
struct comment
{
   ...
   /*
      0: consuming length
      1: consuming key
      2: skipping key or key and value
      3: consuming value 
   */
   int siz;
   char buf[2048];
...
static void parse_comment(vorb *f, comment* com, const char* buf, int len)
{
   while (len)
   {
      switch (com->state)
      {
         case 0:
         {
            int needed = 4-com->siz;
            if (needed <= len)
            {
               memcpy(com->buf+com->siz,buf,needed);
               com->length = *(int*)com->buf;
               com->siz=0;
               com->state = 1;
               len-=needed;
               buf+=needed;
            ...
            {
               memcpy(com->buf+com->siz,buf,len);
               com->siz+=len;
               len = 0;
            }
            break;
         ...
         case 1:
         {
            while (len)
            {
               ...
               if (c=='=')
               {
                  if (com->siz<8 || memcmp(com->buf,"MARKERS=",8))
                  {
                     com->state = 2;
                     break;
                  }
         ...
         case 2:
         {
            // skip till com->siz < com->length
            int needed = com->length - com->siz;
            if (needed <= len)
            {
               com->state = 0;
               buf += needed;
         ...
         case 3:
         {
            ...
            if (needed <= len)
            {
               ...
               {
                  int p,d[6];
                  int n = sscanf(parse, "%d %d %d %d %d %d%n",
                     d+0,d+1,d+2,d+3,d+4,d+5,&p);
                  if (n!=6)
                     break;
...
static int start_decoder(vorb *f)
{
   ...
   */
   ...
   len = next_segment(f);
   bool last_seg = len < 255;
   if (len)
   {
      comment com = {0,0,{0},0};

      // assuming first seg at least encloses:
      // - packet header 
      // - vendor size and string
      // - number of comments

      if (len<15)
         return error(f, VORBIS_invalid_stream);

      if (get8(f) != VORBIS_packet_comment)
         return error(f, VORBIS_bad_packet_type);
      else
      if (!getn(f, header, 6))
         return error(f, VORBIS_unexpected_eof);
      ...
      while (len>0)
      {
         getn(f,(uint8*)raw,len);

         if (last_seg)
            len--;

         parse_comment(f, &com, raw, len);
