...
if [[ "$OSTYPE" == "linux-gnu"* ]]; then
	if [ -f "/usr/bin/time" ]; then
		...
		/usr/bin/time -f "----------------------\ndone in %e sec\n" make -j16 -f makefile_asciicker_server
		echo -e "BUILDING game\n----------------------"
		/usr/bin/time -f "----------------------\ndone in %e sec\n" make -j16 -f makefile_asciicker
		echo -e "BUILDING game_term\n----------------------"
		/usr/bin/time -f "----------------------\ndone in %e sec\n" make -j16 -f makefile_asciicker_term
	else
		make -j16 -f makefile_asciiid
		make -j16 -f makefile_asciicker_server
		make -j16 -f makefile_asciicker
	...
	make -j16 -f makefile_asciicker_server
	make -j16 -f makefile_asciicker_mac
	make -j16 -f makefile_asciicker_term_mac
fi
